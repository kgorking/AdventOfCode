...
int strength(hand_and_bid const& hb) {
	...
	switch (map.size()) {
	case 5: { // high card
		// if one card is joker, it becomes a pair
		return 1 + jokers;
	}
	case 4: { // one pair
		// JJXYZ -> three of a kind
		// JXXYZ -> three of a kind
		return 2 + (jokers ? 2 : 0);
	}
	case 3: {
		// two pair
		...
		// three of a kind
		// JJJXY -> four of a kind
		// JXXXY -> four of a kind
		return 4 + (jokers ? 2 : 0);
	}
	case 2: {
		// full house
		if (3 == *std::ranges::max_element(map | std::views::values)) {
			...
			// JJXXX -> five of a kind
			return 5 + (jokers ? 2 : 0);
		}

		// four of a kind
		// JXXXX -> five of a kind
